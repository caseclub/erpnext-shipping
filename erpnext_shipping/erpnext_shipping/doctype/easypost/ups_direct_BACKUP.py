# apps/erpnext_shipping/erpnext_shipping/doctype/easypost/ups_direct.py
...
class UPSDirect:
    ...
    def get_tracking_data(self, tracking_number: str) -> dict:
        """
        Fetch tracking data for a UPS shipment using EasyPost's tracker API (workaround for 3p auth issues).
        """
        if not self.easypost_key:
            raise frappe.ValidationError("EasyPost API key missing in settings (required for UPS tracking).")
        ...
        if r.status_code not in (200, 201, 409):
            frappe.log_error(
                title="EasyPost UPS Tracking Error",
                message=r.text[:2000] or "(empty)"
            )
            r.raise_for_status()
        ...
        return {
            "awb_number": t.get("tracking_code", tracking_number),
