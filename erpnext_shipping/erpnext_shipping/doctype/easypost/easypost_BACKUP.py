...
class EasyPostUtils:
    ...
    def create_shipment(self, service_info, delivery_address):
        ...
        if (service_info.get("service_provider") or "").upper() == "UPS":
            ...
            try:
                ...
                return {
                    ...
                    "carrier":          "UPS",
                    "carrier_service":  service_info.service_name,
                    "shipment_amount":  resp.get("shipment_amount", service_info.total_price),  # Use actual from response; fallback to quote
        ...
        # ───────────────────────────
        # FedEx 3rd Party Billing (multiple & single parcel) - NEW
        # ───────────────────────────
        if (service_info.get("service_provider") or "").upper() == "FEDEX":
            ...
            try:
                ...
                # Similar to UPS response processing
                tracking_numbers = resp.get("awb_number", "").split(", ")
                label_urls = resp.get("label_bundle", [])  # Renamed for clarity (can be PNG or ZPL)

                if not label_urls:
                    frappe.throw(_("FedEx did not return any labels."))

                # Check if it's ZPL (no PDF conversion needed) or PNG (convert to PDF)
                if label_urls and label_urls[0].endswith('.zpl'):
                    shipping_label = label_urls[0]  # Use ZPL directly as attachment
                else:
                    shipping_label = self._png_to_pdf(label_urls[0]) if len(label_urls) == 1 else self._pngs_to_pdf(label_urls)
                ...
                return {
                    ...
                    "postage_label": {
                        "label_url": shipping_label,  # Universal: ZPL or PDF
                        "label_png_url": label_urls[0] if not is_zpl else None,  # Only for PNG
                        "label_pdf_url": shipping_label if not is_zpl else None,  # Only for PDF-converted PNG
                    },
                }
            except HTTPError as e:
                try:
